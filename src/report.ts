...
export function plain(reason: string): string {
  return reason
    ...
    .replace(/^\[SHA256_MISMATCH\].*$/s, "the downloaded bytes did not match the release's checksum")
    .replace(/^\[SIZE_MISMATCH\].*$/s, "the download was not the size the release says")
    .replace(/^\[MANIFEST_INVALID\].*$/s, "the release manifest is not valid")
    .replace(/^\[[A-Z_]+\]\s*/, "")
