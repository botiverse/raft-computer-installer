...
use k_carrier::{artifact::sha256, state::{OperationRead, Slot}, storage::FileStore};
...
impl World {
    pub fn version(&self) -> Option<&str> {
        match self { Self::Adopted { version } | Self::Managed { version } => Some(version), _ => None }
    }
}
...
fn foreign_manager(binary: &Path) -> Result<Option<&'static str>> {
    ...
    if path.contains("/Cellar/") || path.starts_with("/opt/homebrew/") { return Ok(Some("Homebrew")); }
    if path.contains("/node_modules/") { return Ok(Some("npm")); }
    if path.contains("/.bun/") { return Ok(Some("bun")); }
    ...
    if head[..read].starts_with(b"#!") {
        ...
        if first.lines().next().is_some_and(|line| line.split_whitespace().any(|token| token.ends_with("node") || token.ends_with("bun"))) {
...
fn exists(path: &Path) -> Result<bool> { k_carrier::storage::exists(path) }

fn empty_genesis(cfg: &Config) -> Result<bool> {
    if !exists(&cfg.k_state)? { return Ok(true); }
    for entry in fs::read_dir(&cfg.k_state)? {
        ...
        match name.to_str() {
            ...
            Some("upgrade.lock.claims" | "controllers") if entry.file_type()?.is_dir() => {
                if fs::read_dir(entry.path())?.next().is_some() { return Ok(false); }
            },
            _ => return Ok(false),
        }
    }
...
pub async fn read(cfg: &Config) -> Result<World> {
    ...
    if let Some(manager) = foreign_manager(&cfg.binary)? {
        return Ok(World::Held { reason: format!("the installed program belongs to {manager}") });
    }
    ...
    match &operation {
        OperationRead::Unreadable { .. } => return Ok(World::Broken { reason: "installation records are unreadable".into() }),
        OperationRead::Observed { operation } if operation.outcome.is_none() => {
            return Ok(World::Upgrading { id: operation.id.clone() });
        },
        _ => {},
    }
    let stable = match store.version(Slot::Stable) {
        Ok(stable) => stable,
        Err(_) => return Ok(World::Broken { reason: "stable installation is incomplete".into() }),
    };
    match stable {
        Some(stable) => {
            if version::exact(&stable).is_err() { return Ok(World::Broken { reason: "stable version is invalid".into() }); }
            let state = match store.transaction_state().await {
                Ok(state) => state,
                Err(_) => return Ok(World::Broken { reason: "transaction state is unreadable".into() }),
            };
            if !state.phase().at_rest() {
                return Ok(World::Broken { reason: "transaction journal has unfinished work without an active receipt".into() });
            }
            ...
            match computer::self_report(&cfg.binary, cfg).await {
                ...
                _ => Ok(World::Broken { reason: "installed executable does not report the stable version".into() }),
            }
        },
        None => {
            if !matches!(operation, OperationRead::Genesis) || !empty_genesis(cfg)? {
                return Ok(World::Broken { reason: "installation records have no usable stable slot".into() });
            }
            if !exists(&cfg.binary)? { return Ok(World::Fresh); }
