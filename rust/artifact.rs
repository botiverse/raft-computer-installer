...
pub fn check_installed_sidecar(cfg: &Config, version: &str) -> Result<()> {
    ...
    let dir = cfg.sidecar_dir(version)?;
    if !k_carrier::storage::exists(&dir)? { return Ok(()); }
...
pub async fn acquire(cfg: &Config, manifest: &Manifest) -> Result<PreparedArtifact> {
    ...
    Ok(PreparedArtifact { _directory: directory, path, version: manifest.version.clone(), sidecar_quarantine: None })
}
...
pub async fn acquire_repair(cfg: &Config, manifest: &Manifest) -> Result<PreparedArtifact> {
    ...
    let bytes = downloader.download(&manifest.release, Some(&cfg.scratch()), None).await?;
    check_platform(&bytes)?;
    let directory = tempfile::Builder::new().prefix("repair-candidate-").tempdir_in(cfg.scratch())?;
    let path = directory.path().join(BIN_NAME);
    write_durable(&path, &bytes, true)?;
    let sidecar = match &manifest.sidecar {
        Some(release) => {
            let bytes = downloader.download(release, Some(&cfg.scratch()), None).await?;
    ...
    let record = SidecarRecord {
        format_version: 1, version: manifest.version.clone(),
        sidecar: manifest.sidecar.as_ref().map(|release| SidecarIdentity { sha256: release.sha256.clone(), size: release.size }),
    };
    let cache = cfg.sidecar_dir(&manifest.version)?;
    if let Ok(bytes) = fs::read(cache.join("identity.json")) {
        if let Ok(old) = serde_json::from_slice::<SidecarRecord>(&bytes) {
            if old.format_version == 1 && old.version == manifest.version && old != record {
                return Err(invalid("release sidecar identity changed for an immutable version"));
            }
        }
    }
    ...
    if let Some(bytes) = &sidecar { write_durable(&replacement.join(SIDECAR_NAME), bytes, false)?; }
    ...
    let sidecar_quarantine = if k_carrier::storage::exists(&cache)? {
        ...
        if !meta.is_dir() || meta.file_type().is_symlink() { return Err(invalid("sidecar cache is not an owned directory")); }
        let destination = cfg.installer_dir.join("quarantine").join(format!("sidecar-{}-{}", manifest.version, uuid::Uuid::new_v4()));
        ensure_dir(destination.parent().ok_or_else(|| invalid("sidecar quarantine directory missing"))?)?;
        k_carrier::storage::rename_durable(&cache, &destination)?;
        Some(destination)
    } else { None };
    ensure_dir(cache.parent().ok_or_else(|| invalid("sidecar cache directory missing"))?)?;
    k_carrier::storage::rename_durable(&replacement, &cache)?;
    Ok(PreparedArtifact { _directory: directory, path, version: manifest.version.clone(), sidecar_quarantine })
}
