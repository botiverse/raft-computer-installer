...
use k_carrier::{error::invalid, storage::{now_ms, write_json}};
...
impl Receipt {
    pub fn validate(&self) -> Result<()> {
        if self.protocol != "raft-computer-installer/v3" || self.installer_version.is_empty()
            || self.id.is_empty() || self.id.trim() != self.id || self.id.encode_utf16().count() > 256
            || !["install", "upgrade", "repair", "recover"].contains(&self.operation.as_str())
            || self.exit_code != self.outcome.exit_code() || self.line.trim().is_empty()
            || self.line.len() > 2048 || self.line.chars().any(char::is_control)
            || self.next_step.as_ref().is_some_and(|s| s.len() > 512 || s.chars().any(char::is_control))
        { return Err(invalid("invalid installer receipt")); }
        if let Some(v) = &self.target_version { version::exact(v)?; }
        if let Some(v) = &self.from_version { version::exact(v)?; }
        Ok(())
    }
...
pub fn read(cfg: &Config, id: &str) -> Result<Option<Receipt>> {
    ...
    if bytes.len() > 65536 { return Err(invalid("installer receipt too large")); }
    let receipt: Receipt = serde_json::from_slice(&bytes)?;
    receipt.validate()?;
    if receipt.id != id { return Err(invalid("installer receipt identity mismatch")); }
    Ok(Some(receipt))
}
