...
fn enabled(name: &str) -> bool {
    env::var(name).is_ok_and(|v| {
        !v.is_empty() && !["0", "false", "no"].contains(&v.to_ascii_lowercase().as_str())
    })
}
...
impl Interaction {
    ...
    pub fn detect() -> Self {
        let unattended = Self {
            presence: Presence::Unattended,
            input: None,
            output: None,
        };
        if enabled("CI") || enabled("RAFT_COMPUTER_NON_INTERACTIVE") {
            return unattended;
        }
        #[cfg(unix)]
