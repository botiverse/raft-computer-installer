...
pub async fn ensure(cfg: &Config) -> Result<Option<String>> {
    ...
    let on_path = env::var_os("PATH").is_some_and(|path| env::split_paths(&path).any(|entry| {
        #[cfg(windows)]
        { entry.to_string_lossy().eq_ignore_ascii_case(&directory.to_string_lossy()) }
        #[cfg(not(windows))]
        { entry == directory }
    }));
    if on_path { return Ok(None); }
    let hint = format!("Add {} to PATH.", directory.display());
    if directory != cfg.user_home.join(".local/bin") || env::var("RAFT_COMPUTER_NO_MODIFY_PATH").as_deref() == Ok("1") {
        return Ok(Some(hint));
    }
    #[cfg(unix)]
    {
        ...
        let profile = match shell.rsplit('/').next() {
            Some("zsh") => env::var_os("ZDOTDIR").map(std::path::PathBuf::from).unwrap_or_else(|| cfg.user_home.clone()).join(".zshrc"),
        ...
        if !existing.lines().any(|existing| existing.trim() == line) {
            ...
            let updated = (|| -> std::io::Result<()> {
                if let Some(parent) = profile.parent() { fs::create_dir_all(parent)?; }
                let mut file = OpenOptions::new().append(true).create(true).mode(0o600).open(&profile)?;
                file.write_all(format!("\n# raft-computer\n{line}\n").as_bytes())?;
                file.sync_all()
            })();
            if updated.is_err() { return Ok(Some(hint)); }
        }
    ...
    #[cfg(windows)]
    {
        ...
        let result = crate::computer::run(std::path::Path::new("powershell.exe"), &["-NoProfile", "-NonInteractive", "-Command", script], &environment, std::time::Duration::from_secs(30)).await;
        Ok(Some(if result.is_ok_and(|r| r.success) { "Open a new terminal to use raft-computer.".into() } else { hint }))
    }
}
