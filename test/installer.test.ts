...
describe("records K cannot settle", () => {
  it("reinstalls in the same run, keeping the old records aside", async () => {
    const m = machine();
    ...
    assert.equal(r.code, 0, r.stdout + r.stderr);
    assert.match(r.stdout, /^Reinstalled 1\.1\.0\. The previous installation was kept at .*\. Next: run raft-computer login$/m);
    assert.equal(await m.selfVersion(), "1.1.0");
  });
});
