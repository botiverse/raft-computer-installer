...
import { resolve, join } from 'node:path'; import { readFileSync } from 'node:fs';
...
const files=['cli.cjs','install.sh','install.ps1']; const inventory={schema:'raft-computer-installer/release/v1',installerVersion:JSON.parse(readFileSync('package.json','utf8')).version,files:Object.fromEntries(await Promise.all(files.map(async f=>[f,{sha256:await hash(join(out,f)),size:(await stat(join(out,f))).size}])))};
