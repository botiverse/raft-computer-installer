...
$installerVersion = if ($env:RAFT_COMPUTER_INSTALLER_VERSION) { $env:RAFT_COMPUTER_INSTALLER_VERSION } else { '0.1.0-rc.4' }
...
try {
  ...
  $request = @{ protocol='raft-computer-installer/v1'; installerVersion=$installerVersion; computerVersion=$version; operation='upgrade'; operationId=$id; installDir=$dir } | ConvertTo-Json -Compress
  $request | & (if ($env:RAFT_COMPUTER_INSTALLER_NODE) { $env:RAFT_COMPUTER_INSTALLER_NODE } else { 'node' }) "$tmp/cli.cjs" --validate-request
