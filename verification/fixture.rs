...
use std::{env, fs, io::{BufRead, BufReader, Read, Write}, net::{TcpListener, TcpStream}, path::{Path, PathBuf}, process::{Command, Stdio}, time::{Duration, Instant}};

const fn template() -> [u8; 512] {
    let text = b"{\"version\":\"0.0.0\",\"marker\":\"RAFT_NATIVE_FIXTURE_CONFIGURATION_V1\"}";
    let mut bytes = [0u8; 512];
    let mut i = 0;
    while i < text.len() { bytes[i] = text[i]; i += 1; }
    bytes
}
...
struct Behavior {
    ...
    #[serde(default)] start_fail: bool,
    #[serde(default)] stop_broken: bool,
}

#[derive(Serialize, Deserialize)]
struct State { address: String, token: String, version: String, pid: u32, generation: String }

fn exchange(home: &Path, action: &str) -> Option<Value> {
    let state: State = serde_json::from_slice(&fs::read(home.join("fixture-service.json")).ok()?).ok()?;
    let address = state.address.parse::<std::net::SocketAddr>().ok()?;
    if !address.ip().is_loopback() { return None; }
    ...
    BufReader::new(stream).take(4097).read_to_string(&mut line).ok()?;
...
fn gate(variable: &str) -> Result<(), Box<dyn std::error::Error>> {
    if let Some(path) = env::var_os(variable) {
        ...
        while !path.with_extension("release").exists() {
            if Instant::now() >= deadline { return Err("fixture gate timed out".into()); }
...
fn service(home: &Path, behavior: &Behavior) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let state = State { address: listener.local_addr()?.to_string(), token: uuid::Uuid::new_v4().to_string(),
        version: behavior.live_version.clone().unwrap_or_else(|| behavior.version.clone()),
        pid: std::process::id(), generation: uuid::Uuid::new_v4().to_string() };
    ...
    for stream in listener.incoming() {
        ...
        if BufReader::new(stream.try_clone()?).take(4097).read_line(&mut request).is_err() { continue; }
        let Ok(request) = serde_json::from_str::<Value>(&request) else { continue; };
        if request["token"] != state.token { continue; }
        writeln!(stream, "{}", json!({"generation":state.generation,"pid":state.pid,"version":state.version}))?;
...
fn run() -> Result<u8, Box<dyn std::error::Error>> {
    ...
    if args.first().map(String::as_str) == Some("--version") {
        if env::var_os("RCI_FIXTURE_PUBLISHED_PATH").is_some_and(|path| PathBuf::from(path) == env::current_exe().unwrap_or_default()) {
            gate("RCI_FIXTURE_PUBLISHED_GATE")?;
        }
        println!("{}", behavior.reported_version.as_ref().unwrap_or(&behavior.version));
        return Ok(0);
    }
    let home = PathBuf::from(env::var_os("RAFT_HOME").or_else(|| env::var_os("SLOCK_HOME")).ok_or("fixture home missing")?);
    ensure_dir(&home)?;
    match args.first().map(String::as_str) {
        Some("__service") => service(&home, &behavior)?,
        Some("login") => { fs::write(home.join("fixture-login"), b"configured")?; },
        Some("start") => {
            if !home.join("fixture-login").exists() || behavior.start_fail { return Ok(1); }
            if exchange(&home, "probe").is_some() { return Ok(0); }
            let mut command = Command::new(env::current_exe()?);
            command.arg("__service").stdin(Stdio::null()).stdout(Stdio::null()).stderr(Stdio::null());
            #[cfg(unix)]
            ...
            while exchange(&home, "probe").is_none() {
                if child.try_wait()?.is_some() || Instant::now() >= deadline { return Ok(1); }
                std::thread::sleep(Duration::from_millis(20));
            }
        },
        Some("stop") => {
            gate("RCI_FIXTURE_STOP_GATE")?;
            if behavior.stop_broken { return Ok(0); }
            exchange(&home, "stop");
            let deadline = Instant::now() + Duration::from_secs(5);
            while exchange(&home, "probe").is_some() {
                if Instant::now() >= deadline { return Ok(1); }
                std::thread::sleep(Duration::from_millis(20));
            }
        },
        Some("status") if args.get(1).map(String::as_str) == Some("--json") => {
            ...
            if let Some(live) = live { answer["attestation"] = json!({"servicePid":live["pid"],"computerVersion":live["version"],"serviceGeneration":live["generation"]}); }
            println!("{answer}");
        },
        _ => return Ok(2),
    }
...
fn main() {
    match run() {
        ...
        Err(error) => { eprintln!("fixture: {error}"); std::process::exit(1); },
    }
}
